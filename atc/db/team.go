...
func (t *team) PrivateAndPublicBuilds(page Page) ([]BuildForAPI, Pagination, error) {
	...
	return getBuildsWithPagination(newBuildsQuery, minMaxIdQuery, page, t.conn, t.lockFactory, false, t.elasticsearchClient)
}

func (t *team) BuildsWithTime(page Page) ([]BuildForAPI, Pagination, error) {
	return getBuildsWithDates(buildsQuery.Where(sq.Eq{"t.id": t.id}), minMaxIdQuery, page, t.conn, t.lockFactory, t.elasticsearchClient)
}

func (t *team) Builds(page Page) ([]BuildForAPI, Pagination, error) {
	return getBuildsWithPagination(buildsQuery.Where(sq.Eq{"t.id": t.id}), minMaxIdQuery, page, t.conn, t.lockFactory, false, t.elasticsearchClient)
}
